...
if TYPE_CHECKING:
    ...
    from philly_fair_measure.models.explain import AppealPoint, Explanation
...
def gather(parcel_id: str, data_dir: Path | None = None) -> ReportData:
    ...
    if not is_condo:
        ...
        if screen.get("twin_n") is not None and characteristics:
            from philly_fair_measure.validation.opa import strict_twin_key
    ...
    if not is_condo and characteristics:
        try:
            from philly_fair_measure.models.explain import explain
...
def _render_explanation(exp: Explanation) -> list[str]:
    ...
    from philly_fair_measure.models.explain import plain_language
...
def render_html(data: ReportData) -> str:
    ...
    if data.explanation is not None and c:
        from philly_fair_measure.models.explain import appeal_points
...
def build_property_report(
    ...
    from philly_fair_measure.models.comps import resolve_parcel
