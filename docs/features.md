...
|---|---|---|
| `time_adj_log` | Constant-quality price-index adjustment to the reference month at the most specific geography available, market area, else district, else citywide (marts/price_index.parquet) | Models train on log(price)+adj and drop time features (OPA practice). Fixed the median-ratio drift: 0.92 → **0.99**. |
