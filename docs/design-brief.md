...
## 2. Audience and non-negotiables
...
## 3. Design direction: "civic craft"
...
## 4. Current state (what to improve)
...
1. **No identity.** Needs a wordmark/logotype ("Philly Assessment Check" — or propose a better name treatment) and a favicon/social card. Emoji must be replaced by a small consistent icon set (stroke style, e.g. Lucide, or a bespoke minimal set).
...
## 5. Page inventory (priority order)
...
## 6. Hard technical contract (so the work drops in)
...
3. **Wordmark + favicon + social-card** direction (SVG).
