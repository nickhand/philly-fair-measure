...
web/                    Vue 3 + TypeScript + Vite + Tailwind v4 (mobile-first)
  ...
  src/views/            Home (search), Property (report), Map, AnnualReport,
                        Findings, Methodology, Trust (The Proof), Appeal,
...
## Headline stats and annual-report pipeline
...
The same export builds `annual_report` for `/reports/ty-2027` from the staged
assessment history and current assessment screen. It compares the 2026 and
2027 rolls property by property, reports the change distribution, and labels a
movement `corrective`, `widening`, or `no_clear_change` by the change in
absolute log-distance to the independent estimate (2-log-point deadband).
Known record-quality warnings remain scoreable at the property level but are
excluded from this aggregate direction verdict. The five equal-size value
groups run from the cheapest 20% to the most expensive 20%, based on the
independent estimate rather than either assessment roll. ZIP codes provide the
stable geographic cut without pretending that they are neighborhood boundaries.
The export also joins city-published ACS tract demographics after valuation and
writes the three prespecified majority-race groups under
`annual_report.race_context`. Demographics are audit-only and never enter the
...
common modeled-value range. Run it as a release check before publishing or
updating the annual report.

The Kensington boundary case study is reproduced by the same export. Its
distance bands are computed in
`src/philly_fair_measure/diagnostics/annual_roll.py` from assessment-feature
parcel coordinates: each reliable residential record in ZIPs 19133, 19134,
19140, and 19124 is assigned haversine distance to its nearest reliable record
in the 19122/19125 core proxy. The generated values live under
`annual_report.boundary_transect`; the versioned static SVG renderer is
`web/src/components/viz/BoundaryTransect.vue`. No measured value is entered in
the component. The ZIP and Kensington cuts remain reproducible research views,
but are not on the default public report. Run `fair-measure export-web-stats`
after any screen rebuild to recompute the report and those research views.

This is a release-day, **provisional** diagnostic rather than a TY2027
sales-ratio study: the roll is effective January 1, 2027, and no transactions
under that effective date exist yet. The report publishes the model benchmark
date and OPA sales cutoff, presents OPA's own passing citywide study alongside
the disaggregated result, and promises post-effective-date updates rather than
calling the current model ground truth.
